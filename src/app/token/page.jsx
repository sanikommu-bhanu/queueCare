  ...
  const refresh = useCallback(async () => {
    ...
    if (tokenId.startsWith('demo-')) {
      setToken(prev => {
        ...
        if (status==='called' && prev.status!=='called') {
          pushNotification({ title:'🔔 Your turn!', body:`Token #${pad(prev.token_number)} called at ${prev.clinic_name}`, type:'called' });
          toast.success('Your token has been called! 🔔');
          try { window.speechSynthesis?.speak(new SpeechSynthesisUtterance(`Token number ${prev.token_number}, please proceed to the consultation room.`)); } catch {}
        }
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-safe">
      ...
      <div className="px-5 pt-5">
        ...
        {/* Actions */}
        <button onClick={()=>{ try { window.speechSynthesis?.speak(new SpeechSynthesisUtterance(`Your token is ${token.token_number}. ${token.tokens_ahead} patients ahead. Estimated wait is ${token.estimated_wait_minutes} minutes.`)); } catch { toast.error('Speech not available'); } }}
